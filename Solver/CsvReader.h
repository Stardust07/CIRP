...
#define SMART_QYM_INVENTORY_ROUTING_CSV_READER_RECURSIVE_VERSION  0
...
public:
    ...
    const std::vector<Row>& scan(std::ostringstream &oss) {
        ...
        #if SMART_QYM_INVENTORY_ROUTING_CSV_READER_RECURSIVE_VERSION
        onNewLine(begin);
        #else
        onNewLine_opt(begin);
        #endif // SMART_QYM_INVENTORY_ROUTING_CSV_READER_RECURSIVE_VERSION
